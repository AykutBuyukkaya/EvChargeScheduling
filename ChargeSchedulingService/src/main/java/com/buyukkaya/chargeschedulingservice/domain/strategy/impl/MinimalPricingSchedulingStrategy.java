...
public class MinimalPricingSchedulingStrategy implements SchedulingStrategy {
    ...
    public MinimalPricingSchedulingStrategy(@Qualifier("minimalPricingChain") DataCollectorService dataCollectorService, DataCollectorMapper dataCollectorMapper, ScheduledChargeMapper scheduledChargeMapper) {
    ...
    private void findOptimalPricing(DataCollectorResponse dataCollectorResponse, ScheduledCharge scheduledCharge) {
        ...
        LocalTime minIndex = null;
        Double minChargingCost = null;

        for (LocalTime ll = scheduledCharge.getArrivalTime(); ll.plusMinutes(requiredTimeToChargeInMinutes).isBefore(scheduledCharge.getDepartureTime()); ll = ll.plusMinutes(1)) {
            LocalTime finalLl = ll;
